...
- **Schedule the regrade explicitly** — write it into `state.json.parking_lot` and into the journal's next-session seed, so the next `/standup` surfaces it automatically.
...
- on an `overall: pass` that clears a module gate: set `gates.<module>.status` to `passed`, and note that the module advance also requires the portfolio write-up (§CONCEPT 2.7 — the write-up is inside the gate, not after it). Do not advance `meta.current_module` until both are true.
