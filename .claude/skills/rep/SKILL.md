...
Verify your own test file runs and fails cleanly against the stub before handing it over. An exercise whose test errors on import wastes the student's session.
...
Then note the start time and get out of the way. Do not narrate. Do not check in every two minutes. Observing means observing.
...
A rep ends when the tests pass **or** when the time is up. Both are legitimate endings. Run the same debrief either way:
...
- `last_touched` = today
- `evidence` — factual and specific: `"rep-003 unassisted: passed 5/6, failed the empty-input case, rung-2 hint on the generator"`
